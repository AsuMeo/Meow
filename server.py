...
<body>
    <div class="container">
        ...
        <div class="info-box">
            💡 <strong>Твой голос детский</strong> — поэтому настройки оптимизированы. 
            Pitch +6, Formant +4, Speed 1.15x — идеально для превращения в девочку 14-16 лет.
            <br><br>
            🔒 <strong>Безопасно:</strong> оригинал невозможно восстановить из результата.
        </div>
...
def apply_voice_change(audio_path, pitch_shift=6, formant_shift=4, speed=1.15, cute=80):
    """Превращает мужской/детский голос в женский/девочку."""
    ...
    f0_new[f0 == 0] = 0  # сохраняем беззвучные участки

    # Formant shift (тембр/резонансы)
    # Сдвигаем спектральную огибающую
    fft_size = pw.get_cheaptrick_fft_size(sr, f0)
    ...
    # 3. Дополнительный pitch shift через librosa (тонкая подстройка)
    y_shifted = librosa.effects.pitch_shift(
        y_world.astype(np.float32), sr=sr, n_steps=pitch_shift * 0.3
    )

    # 4. Педалборд — финальная обработка
    cute_factor = cute / 100.0
    board = Pedalboard([
        HighpassFilter(cutoff_frequency_hz=180 + cute_factor * 120),  # убираем низ
        LowShelfFilter(cutoff_frequency_hz=250, gain_db=-3 - cute_factor * 4),  # меньше баса
        HighShelfFilter(cutoff_frequency_hz=3000, gain_db=2 + cute_factor * 4),  # больше верха
        Compressor(threshold_db=-20, ratio=3.0),  # выравниваем громкость
        Gain(gain_db=2),  # немного громче
    ])
